"""
AI 면접 Chain mock service.

배치 위치:
backend/apps/interview/services/ai_chain_service.py

역할:
- DB/LLM 연결 전에도 API 흐름을 검증할 수 있는 deterministic mock 로직 제공
- 나중에 LLM Chain으로 교체해도 view/API 계약은 유지할 수 있도록 service layer 분리
"""
...
class InterviewAIChainService:
    """질문 생성, 답변 충족도 판단, 꼬리질문 생성을 담당하는 서비스."""
    ...
    def generate_questions(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        for index in range(1, question_count + 1):
            ...
            question_text = self._build_question_text(index, question_type, input_sources, fallback_used)
            ...
            questions.append(
                {
                    ...
                    "difficulty": None,
                    "order_index": index,
                    "generation_reason": self._build_generation_reason(source_tags, fallback_used),
                    "source_tags": source_tags[:2] if options.get("allow_multiple_source_tags", True) else source_tags[:1],
                }
            )
    ...
    def generate_followup_mock(self, payload: dict[str, Any]) -> dict[str, Any]:
        """답변 충족도 판단 → 꼬리질문 생성까지 한 번에 검증하는 mock API용 서비스."""
        answer = payload.get("answer") or {}
        ...
        sufficiency_payload = {
            ...
            "weakness_tag_candidates": payload.get("weakness_tag_candidates") or DEFAULT_WEAKNESS_TAG_CANDIDATES,
        }
        ...
        # 실제 DB 연동 시에는 여기서 answer_weakness_tags 저장 후 생성 id를 사용하고,
        # interview_questions에 꼬리질문을 저장한 뒤 question_id를 내려준다.
        followup_question = {
    ...
    def _build_source_tags(self, input_sources: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if jd:
            excerpt = jd.get("job_requirements") or jd.get("original_text") or ", ".join(jd.get("keywords") or [])
            tags.append(
                {
        ...
        if resume:
            ...
            if projects:
                ...
                excerpt = ", ".join([skill.get("skill_name", "") for skill in skills if isinstance(skill, dict)])
            tags.append(
                {
            ...
            tags.append(
                {
                    ...
                    "source_text_excerpt": self._shorten(first_item.get("answer_text") or first_item.get("question")),
                }
            )
