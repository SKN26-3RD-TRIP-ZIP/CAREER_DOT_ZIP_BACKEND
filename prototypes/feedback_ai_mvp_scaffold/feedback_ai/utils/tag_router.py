# prototypes/feedback_ai_mvp_scaffold/feedback_ai/utils/tag_router.py
...
def route_deterministic_tags(
    ...
    cbi_level = cbi_competency_root.get("assigned_level", 1)
    is_grounded = grounding_res.get("is_grounded", False) if grounding_res else False

    # P0-4 기획(매칭률 20% 미만) 구현을 위한 아키텍처 트레이드오프 마스터 사전
    ...
    tradeoff_match_ratio = len(matched_tradeoff_words) / len(TRADEOFF_DICTIONARY)
    ...
    # P0-1. weak_question_relevance (외부 실시간 레이어 검증 데이터와 바인딩)
    ...
    # P0-2. weak_specificity (행동 15점 미만, 결과 12점 미만 면접관 가이드라인 과락선 준수)
    ...
    # P0-3. weak_technical_understanding (Grounding 과락 보정치인 40점 컷오프 싱크 완료)
    ...
    # P0-4. weak_technical_reasoning (점수 대신 기획 원안의 아키텍처 사전 매칭률 20% 미만 수식 적용)
    ...
    # P0-6. weak_evidence (Grounding 완전 실패 지표 연동)
    if not is_grounded:
        triggered_weaknesses.append({
    ...
    # P0-7. weak_jd_fit (외부 실시간 레이어 검증 데이터와 바인딩)
    if llm_weakness_tags and any(t.get("tag_name") == "weak_jd_fit" for t in llm_weakness_tags):
        # 외부 LLM이 매칭해준 구체적인 실패 사유가 있다면 가져오고, 없으면 기본 디스크립션 사용
    ...
    # P0-8. weak_problem_solving_process (키워드 검색 및 유령 플래그 전면 제거 -> CBI 루브릭 1, 2레벨 통합 매핑)
    if cbi_level <= 2:
        triggered_weaknesses.append({
    ...
    # P0-9. weak_result_impact (성과 확정 어휘 유실 검증 규칙)
    result_desc = bei_star.get("result", {}).get("desc", "")
    ...
    # P0-10. weak_answer_structure (표지어 필터 제거 -> [장황함 대비 구조 스코어 붕괴] 알고리즘 적용)
    ...
    # P0-11. excessive_filler_words (evaluation_service 와 동일하게 total_filler > 6 기준 바인딩)
    ...
    # P0-12. frequent_long_pauses (3초 이상 침묵 임계치)
    ...
    # P0-13. unbalanced_speech_pace (음성 비언어 지표 매핑)
    ...
    # ----------------------------------------------------
    # 🌟 [강점 태그] 안정화 루프 (P0-1 ~ P0-14 명세 보완)
    # ----------------------------------------------------
    
    # P0-1. sharp_problem_definition (CBI 역량 상위권 충족)
    if cbi_level >= 4:
    ...
    # P0-2. data_driven_achievement (실제 정량 지표 매핑 성공 규칙)
    has_numeric_metrics = bool(re.search(r'\d+(%|건|배|명|개|원|ms|초)', result_desc))
    ...
    if question_type == "technical" and tech_score >= 90:
        triggered_strengths.append({
            ...
            "category": "technical",
            "description": "단순 라이브러리/툴 사용법을 넘어 프레임워크나 아키텍처의 내부 동작 원리까지 깊이 있게 파악하고 설명함",
    ...
    # P0-5. top_down_delivery (초반 3문장 이내 성과 위주 진술 구조 파싱)
    ...
    # P0-6. well_balanced_tradeoff (아키텍처 사전 매칭률이 20% 이상 고루 포착될 때)
    ...
    if growth_match_count >= 2 and bei_result_score > 20:  # 👈 기존 15점에서 '20 초과'로 대폭 상향
        triggered_strengths.append({
    ...
    if bei_situation_score > 20 and len(bei_star.get("situation", {}).get("desc", "")) >= 100:
        triggered_strengths.append({
    ...
    action_desc = bei_star.get("action", {}).get("desc", "")
    action_keywords = ["리팩토링", "구축", "구현", "최적화", "설계", "마이그레이션", "도입"]
    if bei_action_score > 20 and any(ak in action_desc for ak in action_keywords):
        triggered_strengths.append({
    ...
    if not triggered_weaknesses and bei_situation_score > 20 and bei_action_score > 20:  # 👈 '각 20 초과' 조건 적용
        triggered_strengths.append({
